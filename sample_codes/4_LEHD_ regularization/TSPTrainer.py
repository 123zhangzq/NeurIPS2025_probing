    ...
    def _train_one_batch(self, episode,batch_size,epoch):
        ...
        while not done:
            if current_step == 0:
                ...
                selected_teacher, prob, probs, selected_student, loss_h_l1 = self.model(state, self.env.selected_node_list, self.env.solution, current_step, flg_out_h=True)  # 更新被选择的点和概率
                loss_mean = -prob.type(torch.float64).log().mean()

                L1_lambda = 1e-6
                loss_mean = loss_mean + L1_lambda * loss_h_l1
