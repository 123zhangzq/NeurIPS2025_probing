...
class TSPModel(nn.Module):
    ...
    def forward(self, state, selected_node_list, solution, current_step,repair = False, flg_out_h = False):
        # solution's shape : [B, V]
        batch_size_V = state.data.size(0)

        if self.mode == 'train':

            probs, loss_h_l1  = self.decoder(self.encoder(state.data), selected_node_list,  flag_out_h = True)
        ...
        if self.mode == 'test':
            ...
            if  repair == True :
                ...
                prob = 1
        if flg_out_h == True:
            return selected_teacher, prob, 1, selected_student, loss_h_l1
...
class TSP_Decoder(nn.Module):
    ...
    def forward(self,data,selected_node_list, flag_out_h=False):
        ...
        ################# Regularization ####################
        out_h = self.l1_sparsity_loss(out.clone(), reg_mode='token')
        ################# Regularization END ####################
        ...
        if flag_out_h == True:
            return new_props, out_h
        return new_props

    def l1_sparsity_loss(self, h, reg_mode='token', p=1):
        if reg_mode == 'global':
            return torch.mean(torch.abs(h))
        elif reg_mode == 'token':
            return torch.norm(h, p=p, dim=-1).mean()
        else:
            raise ValueError("mode must be 'global' or 'token'")
